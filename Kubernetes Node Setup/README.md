## Prerequisites for Installing a Kubernetes Cluster( master + worker )
...
**Reminder:** If you set up in **windows** then follow this step:
* Go to **windows control panel** -> Network and sharing Center -> Change Adapter settings -> Check your wifi status(Intel Dual Brand AC something).
* Then go to vmware -> edit -> virtual network editor -> change setting -> Bridge network with adding your wifi (e.g. Intel something). 

## 1. Setting up Static IPV4 on all nodes (Master & Worker Node).
* No need this when you setup with using manual ip in your node.
...
```
Apply changes: only for above code\
`sudo netplan apply`
...
# Common scenarios(if you faced in setup then follow this)
